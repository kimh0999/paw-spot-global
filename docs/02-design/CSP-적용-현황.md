...
작성 2026-09-11 · 갱신 2026-09-12(Next 16 이행 · 정적 생성 전제 정정) · 정책 정의 `next.config.mjs`
...
1. **nonce** — proxy(옛 middleware)에서 요청마다 nonce를 만들어 CSP 헤더에 넣는다.
   ~~대가: 정적 생성 28페이지를 잃는다.~~ → **2026-09-12 정정. 잃을 정적 생성이 없다.**
   아래 §4-1 참조 — 빌드 산출물을 직접 세어 보니 미리 만들어지는 페이지는 처음부터
   없었다. 이 항목은 더 이상 nonce 도입의 걸림돌이 아니다.
...
| 6 | ~~nonce ↔ 정적 생성 28페이지의 교환 결정~~ | ✅ **2026-09-12 해소 — 교환 자체가 없었다**(§4-1) |
| 7 | nonce를 고르면 `src/proxy.ts`에 붙인다. 렌더 방식은 빌드 출력이 아니라 **`prerender-manifest.json`과 `.next/server/app`의 `.html` 개수**로 확인한다(빌드 출력의 `● SSG` 표시는 Next 15에서 실제 산출물과 달랐다). **nonce 전파·스트리밍·`next/script` 동작을 새로 확인해야 한다** | ⬜ 미완 |
| 8 | 위반 수집 엔드포인트 마련 (§3-7) | ⬜ 미완 |
| 9 | 헤더 이름을 `Content-Security-Policy`로 변경 | ⬜ 미완 |

### 4-1. nonce가 영향을 주는 경로 — **2026-09-12 전면 정정**

이전 판의 이 절은 "빌드가 미리 만드는 28개 페이지가 대상"이라고 적었다. **틀렸다.**
근거로 삼은 것은 빌드 출력의 `Generating static pages (28/28)` 줄과 경로 표의 `●(SSG)`
표시였는데, 둘 다 **실제 산출물이 아니다** — 앞의 것은 정적 생성을 *시도한* 진행 카운터이고,
뒤의 것은 Next 15가 `generateStaticParams`가 있다는 이유로 붙인 표시다.

Next 16 이행 작업에서 산출물을 직접 세어 확인했다.

| 확인 대상 | Next 15.5.25 | Next 16.3.5 |
|---|---|---|
| `prerender-manifest.json`의 `routes` | `/favicon.ico` **1건** | `/favicon.ico`, `/_global-error` **2건** |
| `.next/server/app` 아래 `.html` | **0개** | **1개** (`_global-error.html`) |
| 경로 표의 `●(SSG)` 표시 | 13개 | **0개** (전부 `ƒ`) |

즉 **사용자에게 정적 HTML로 서빙되던 페이지는 처음부터 하나도 없었다.**
루트 레이아웃(`src/app/layout.tsx`)이 `await getLocale()`로 요청 헤더를 읽기 때문에
모든 경로가 요청 시 렌더된다. Next 16의 경로 표는 그 사실을 그대로 표시할 뿐이고,
Next 15의 `●` 표시가 실제를 가리고 있었다.

**따라서 nonce 도입의 대가로 계산했던 "정적 생성 상실"은 존재하지 않는다.**
검토해야 할 것은 남아 있다 — nonce 값이 스트리밍된 청크와 `next/script`까지 제대로
전파되는지, 그리고 CDN 앞단 캐시 전략(현재 응답은 이미
`Cache-Control: private, no-cache, no-store, max-age=0, must-revalidate`다).
