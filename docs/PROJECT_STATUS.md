...
| 서비스 범위 안내 | 완료 (2026-09-07) | 범위 판정은 **필터를 거치지 않은** 공개 장소까지의 최단 거리 > 50km(`isOutsideServiceArea`). 거리를 아는 장소가 없으면 판정하지 않는다. 범위 안이면 상시 안내 배너, 범위 밖이면 Amber 배너 + `대전 장소 보기`. 버튼은 `lat`·`lng`와 `sort=distance`를 주소에서 걷어내고, 지도는 **위치를 껐을 때만** 대전으로 되돌린다 | — | — | `service-area.ts`, `PlacesClient.tsx`, `useUserLocationQuery.ts` |
...
| #20 모바일 시트에서의 범위 배너 | 데스크톱 폭에서만 확인. 창 리사이즈가 뷰포트에 반영되지 않아 시트 레이아웃 미확인 |
...
| 지도 `Google 지도를 제대로 로드할 수 없습니다` 오버레이 | Google Cloud Console (키·결제 상태) | 지도 | **2026-09-07 재현.** 타일은 뜨지만 오류 오버레이가 함께 뜬다. 이번 작업과 무관 — 중심 좌표만 바꿨다 |
| `데오` 장소 좌표 | 관리자 화면에서 좌표 확인 | 데이터 | **2026-09-07 발견.** 주소는 `대전 서구 도안북로93번길`인데 서울 기준 거리가 390.7km로 나온다(대전은 ~140km). 좌표가 틀렸을 가능성이 높다 |
